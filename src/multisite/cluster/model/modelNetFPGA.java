...
public class modelNetFPGA {
	...
	public Double getBWFromPortState(String link, String state) {
		Double bw = 0.0;
		switch (state) {
		case PORT_IDLE: {
			bw = getAvailableBandwidth(link);
			break;
		}
		case PORT_10: {
			bw = 10 - (1000 - getAvailableBandwidth(link));
			break;
		}
		case PORT_100: {
			bw = 100 - (1000 - getAvailableBandwidth(link));
			break;
		}
		case PORT_1G: {
			bw = getAvailableBandwidth(link);
		...
		try {
			...
			while (rs.next()) {
				...
				if(state.equals(PORT_IDLE))
					pwcons=pwcons+getPower(PORT_IDLE);
				if (state.equals(PORT_10)||state.equals(PORT_100)||state.equals(PORT_1G))

					pwcons = pwcons + getPower(PORT_1G);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		try {
			...
			while (rs.next()) {
				...
				if (state.equals(NODE_10_100))
					powercorestatic = powercorestatic + PWCORE10_100
							+ PWCORESTATIC;
//				if (state.equals(NODE_IDLE))
//					powercorestatic = powercorestatic + PWCOREIDLE
//							+ PWCORESTATIC;
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public Integer powerCoreStatic1G() {
		int powercorestatic = 0;
		database = new Database();
		conn = database.connect();
		...
		try {
			...
			while (rs.next()) {
				...
				if (state.equals(NODE_1G))
					powercorestatic = powercorestatic + PWCORE1G + PWCORESTATIC;
				if (state.equals(NODE_10_100))
					powercorestatic = powercorestatic + PWCORE1G
							+ PWCORESTATIC;
//				if (state.equals(NODE_IDLE))
//					powercorestatic = powercorestatic + PWCOREIDLE
//							+ PWCORESTATIC;
...
//							+ PWCORESTATIC;
	...
	public Integer powerFullMesh(int numlink, int numnode) {
		...
		Integer pw = numlink * PWPORT1G + numnode * PWCORE1G + PWCORESTATIC
				* numnode;
		return pw;
	}
	public Integer powerNodeOptimize()
	{
		Integer power=0;
		power =powerCoreStatic1G()+powerPort1G();
		return power;
	}
	public Integer powerNodeandLinkOptimize()
	{
		Integer power=0;
		power =powerCoreStatic()+powerPort();
		return power;
	}
}
